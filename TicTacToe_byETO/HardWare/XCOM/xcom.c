...
void usart_xcom_init(void)
{
	...
	gpio_mode_set(USART_xcom_CS_PORT , GPIO_MODE_OUTPUT , GPIO_PUPD_PULLDOWN , USARTX_xcom_CS_Pin);
	gpio_output_options_set(USART_xcom_CS_PORT , GPIO_OTYPE_PP , GPIO_OSPEED_60MHZ , USARTX_xcom_CS_Pin);
...
void usart_xcom_send_str(uint8_t* str , uint8_t len)
{
	...
	usart_xcom_CS(USARTX_xcom_Send);
...
void usart_xcom_recv_buf(void)
{
	if (recv_flag)
	{
		...
		usart_xcom_send_str(recv_real_buf , recv_real_len);
		recv_flag = 0;
	}
}
...
void usart_xcom_CS(uint8_t cs)
{
	if (cs == 1)
	{
		gpio_bit_set(USART_xcom_CS_PORT , USARTX_xcom_CS_Pin);
	}
	else
	{
		gpio_bit_reset(USART_xcom_CS_PORT , USARTX_xcom_CS_Pin);
	}
}
...
void USART1_IRQHandler(void)
{
	...
	if (usart_interrupt_flag_get(USART , USART_INT_FLAG_TC))
	{
		...
		usart_xcom_CS(USARTX_xcom_Receive);
		usart_interrupt_enable(USART , USART_INT_RBNE);
		usart_interrupt_enable(USART , USART_INT_IDLE);
	}
