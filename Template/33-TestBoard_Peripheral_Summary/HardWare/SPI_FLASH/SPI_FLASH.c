...
/*!
	\brief      initialize SPI4 GPIO and parameter
	\param[in]  none
	\param[out] none
	\retval     none
*/
void spi_flash_init(void)
{
	...
	// /* SPI4_CLK(PH6), SPI4_MISO(PF8), SPI4_MOSI(PF9) */
	...
	// /* SPI4_CS(PF10) GPIO pin configuration */
	...
	// spi_init_struct.prescale = SPI_PSC_16;		//300 / 16 = 18MHz
...
uint16_t spi_flash_send_halfword(uint16_t half_word)
{
	...
	while (RESET == spi_i2s_flag_get(SPI4 , SPI_FLAG_TXF));

	/* send half word through the SPI4 peripheral */
	spi_i2s_data_transmit(SPI4 , half_word);

	/* wait to receive a half word */
	while (RESET == spi_i2s_flag_get(SPI4 , SPI_FLAG_RWNE));
